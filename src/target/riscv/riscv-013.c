...
{
	if (!LOG_LEVEL_IS(LOG_LVL_DEBUG_IO)) /* ESPRESSIF */
		return;
...
access_memory_sysbus(struct target *target, const struct riscv_mem_access_args args)
{
	...
	if (r->mem_access_sysbus_cache_sync && r->cache_writeback)
		r->cache_writeback(target, args.address, args.size * args.count);
	const bool is_read = riscv_mem_access_is_read(args);
	...
	/* Espressif */
	if (!is_read && r->mem_access_sysbus_cache_sync && r->cache_invalidate)
		r->cache_invalidate(target, args.address, args.size * args.count);
